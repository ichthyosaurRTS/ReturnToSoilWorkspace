    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        ...
        if (worldIn.getRawBrightness(pos, 0) >= 9) {
            ...
            if (i < this.getMaxAge()) {
                ...
                {
                    if (worldIn.getBlockState(pos.below()).is(Blocks.FARMLAND)) {worldIn.removeBlock(pos,false);
                        popResource(worldIn, pos, new ItemStack(BlockItemInit.LILYPAD_LANTERN_SEED.get(),1));} //destroy the block if on farmland
                    else { worldIn.setBlock(pos, this.nextAgeWithRotationWithLit(state,i+1), 2);}
