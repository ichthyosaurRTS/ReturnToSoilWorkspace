...
public class DragonflyEntity extends MonsterEntity {
    ...
    private void updateWingDegree(){
        ...
        else this.entityData.set(TAIL_DEGREE1, deg2+5);

        int deg3 = this.entityData.get(TAIL_DEGREE2);
        if (deg3>180) this.entityData.set(TAIL_DEGREE2, 0);
        else this.entityData.set(TAIL_DEGREE2, deg3+5);
    }
    ...
    public int[] getTailDegree() {
        ...
        int deg = this.entityData.get(TAIL_DEGREE1);
        if (deg<45) array[0] = deg;
        else if (deg<91) array[0] = 90-deg;
        else if (deg<135) array[0] = 90-deg;
        else array[0] = deg-180;

        int deg2 = this.entityData.get(TAIL_DEGREE2);

        if (deg2<45) array[1] = deg2;
        else if (deg2<91) array[1] = 90-deg2;
        else if (deg2<135) array[1] = 90-deg2;
        else array[1] = deg2-180;
    ...
    public ILivingEntityData finalizeSpawn(IServerWorld p_213386_1_, DifficultyInstance p_213386_2_, SpawnReason p_213386_3_, @Nullable ILivingEntityData p_213386_4_, @Nullable CompoundNBT p_213386_5_) {
        ...
        this.entityData.set(TAIL_DEGREE1, 50);
        return p_213386_4_;
    }
